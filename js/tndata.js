...
function generateComplaints() {
  ...
  const T = {
    "Road & Infrastructure":["Pothole on main road","Road repair needed","Broken footpath","Road flooding issue","Speed breaker damaged","Road divider broken","Median damaged"],
    "Water Supply & Distribution":["No water supply for 3 days","Contaminated water","Broken pipe leaking","Low water pressure","Water meter fault","Water tank overflow"],
    "Electricity & Power":["Power outage since morning","Transformer fault","Loose wires hanging","Electric pole damaged","No power in street","Frequent voltage fluctuation"],
    "Sanitation & Waste Management":["Garbage not collected","Overflowing bin","Illegal dumping","Dead animal on road","Open defecation area","Sewage overflow"],
    "Street Lights":["Street light not working","Bulb broken","Light pole fallen","Wiring exposed on pole","No lights in area"],
    "Drainage & Flooding":["Blocked drain causing flood","Overflowing sewage","Drain needs cleaning","Open manhole","Storm drain blocked"],
    "Parks & Public Spaces":["Park bench broken","Children play area unsafe","Garden maintenance needed","Lights in park not working","Encroachment in park"],
    "Public Health & Hygiene":["Mosquito breeding in area","Stagnant water","Illegal slaughterhouse","Hospital waste dumping","Food vendor unhygienic"],
    "Building & Construction Violations":["Illegal construction","Building without permit","Encroachment on footpath","Wall collapse risk","Unsafe scaffold"],
    "Noise Pollution":["Loud speaker at night","Factory noise","DJ music till late","Vehicle horn misuse","Construction noise"],
    "Stray Animals":["Stray dogs attacking","Stray cattle on road","Animal carcass on road","Cattle in market area","Wild animals near school"],
    "Environment & Pollution":["Factory smoke polluting","Burning waste openly","Chemical discharge in canal","River bank dumping","Air pollution from kiln"],
    "Traffic & Transportation":["No traffic signal","Road marking faded","Encroachment on junction","Bus stop in bad condition","Auto stand blocking road"],
    "Education & Schools":["School building in bad shape","Toilet not working in school","Drinking water issue in school","Mid-day meal quality issue","School boundary wall broken"],
    "Healthcare Facilities":["PHC closed unexpectedly","Medicine shortage","Nurse absent","Ambulance not available","Hospital waste not disposed"],
    "Revenue & Property Issues":["Patta transfer pending","Encroachment on patta land","Survey boundary dispute","Title deed correction needed"],
    "Encroachment":["Pavement encroachment by shop","Road side stall blocking traffic","Hawker blocking entrance","Compound wall on public land"],
    "Others":["Tree fallen on road","Banner blocking view","Hawker encroachment","Unauthorized sign board","Other civic issue"]
  };
  const S = ["open","open","open","inprogress","inprogress","resolved"];
  const P = ["low","medium","medium","high","urgent"];
  TN_DISTRICTS.forEach(d => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const priority = P[Math.floor(Math.random()*P.length)];
      const date  = new Date(now - Math.floor(Math.random()*45)*86400000);
      const titles= T[cat] || T["Others"];
      const pin   = getWardPincode(ward) || '';
      complaints.push({
        id: id++, title: titles[Math.floor(Math.random()*titles.length)],
        category: cat, district: d.name, districtId: d.id, ward,
        pincode: pin, priority, status,
        lat: d.lat+(Math.random()-.5)*.28, lng: d.lng+(Math.random()-.5)*.28,
        description: `Reported issue in ${ward}, ${d.name}. Requires attention.`,
        reportedBy: `Citizen #${Math.floor(Math.random()*9000)+1000}`,
        phone: `9${Math.floor(Math.random()*900000000)+100000000}`,
        userId: null, geoTagged: false, updates: [],
        createdAt: date.toISOString(),
        updatedAt: status!=='open' ? new Date(date.getTime()+Math.random()*5*86400000).toISOString() : date.toISOString(),
        resolution: status==='resolved' ? 'Issue addressed by concerned department.' : ''
      });
    }
  });
  return complaints;
